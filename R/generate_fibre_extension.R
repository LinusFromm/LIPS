#' Compute a specific p-fibre extension
...
#' @export
generate_fibre_extension <- function(A,
                                     y,
                                     p,
                                     epsilon = 1e-04){
  x_max = c()
  ...
  coords = apply(rbind(x_max, x_min, p)[,hyperrectangle_ext_coord], 2, function(col) return((col[2]-col[3]):(col[1]+length(hyperrectangle_ext_coord)*col[3])), simplify = FALSE)
  ...
  fibre_idx = which(apply(ext, 2, function(col) all(round(col) >= -p[hyperrectangle_ext_coord]) && all(abs(col - round(col)) <= epsilon)))
